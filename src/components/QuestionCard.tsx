...
interface QuestionCardProps {
  question: string;
  options?: string[]; // options are made optional or ensure it's always provided
  correctAnswer: string;
  onOptionSelect: (selOpt: string) => void; 
}

const QuestionCard: React.FC<QuestionCardProps> = ({ question, options, correctAnswer, onOptionSelect }) => {
  ...
  return (
    <div className="bg-white mr-10 shadow-md rounded-lg p-6 my-4 border border-gray-200">
      ...
      <div className='ml-2'>
        <h3 className='text-xl font-semibold mb-6 mt-6 '>{question}</h3>
      </div>
      <div className="space-y-2 mb-6 pb-4 border-b-2 border-gray-200">
        {options.map((opt, idx) => (
          <button
            ...
            className={`w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 rounded-lg 
              ${opt === correctAnswer ? 'bg-gray-100 border border-green-500' : ''}`}
          >
