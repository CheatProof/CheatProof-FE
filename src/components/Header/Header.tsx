...
import {  StickyNote, CircleHelp, Library, } from "lucide-react";
import Sidebar, { MultilevelSidebarItem, SidebarItem, } from "../Sidebar1"
import { FaBackward } from "react-icons/fa6";

const Sidebar2 = ({ name, page, id }: any) => {

  return (
    <>
      <div className="flex sticky top-0">
        <Sidebar name={name} page={page} >
          {/* <SidebarItem icon={<Home size={20} />} text="Home" alert /> */}
          {/* <SidebarItem icon={<LayoutDashboard size={20} />} to="/teacher-dashboard" text="Add Question" active /> */}
          <MultilevelSidebarItem icon={<StickyNote size={20} />} text="Add Questions"   >
          {[
            { icon: <StickyNote size={20} />, text: "Create Question" ,to:'/teacher-dashboard/createQuestion?testId='+id},
            { icon: <CircleHelp size={20} />, text: " Reuse from Question Bank",to:`/teacher-dashboard/test/test-editor/question-bank/${id}` },
            { icon: <Library size={20} />, text: "Select Random Question",to:`/teacher-dashboard/test/test-editor/question-bulk-list/${id}` },
            { icon: <Library size={20} />, text: "Import Spreadsheet",to:'/teacher-dashboard/importQuestion' },
          ]}
          </MultilevelSidebarItem>
          {/* <MultilevelSidebarItem icon={<Users size={20} />} text="Groups"   >
          {[
            { icon: <Users size={20} />, text: "All Groups" ,to:'/teacher-dashboard/allgroups'},
            { icon: <SquareArrowOutDownRight size={20} />, text: "Export" },
            { icon: <Library size={20} />, text: "Statistics" },
          ]}
          </MultilevelSidebarItem> */}
          <SidebarItem icon={<FaBackward size={20} />} text="Back" to={-1} active/>
